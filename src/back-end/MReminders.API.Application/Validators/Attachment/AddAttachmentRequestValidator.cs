...
public class AddAttachmentRequestValidator : AbstractValidator<AddAttachmentRequest>
{
    ...
    public AddAttachmentRequestValidator(IReminderService reminderService)
    {
        ...
        RuleFor(x => x.FileName).NotNull().NotEmpty().WithMessage("The ReminderId must not be null or empty").WithErrorCode("400"); 
        RuleFor(x => x.FileName).Must(ContainOnlyLettersAndDigitsStartingByALetter).WithMessage("The ReminderId must contain only Letters and Digits starting by a letter with 50 chars limitation").WithErrorCode("400"); 
    ...
    private bool ContainOnlyLettersAndDigitsStartingByALetter(string name) => new Regex("^[a-zA-Z]{1}[a-zA-Z0-9]{49}$").IsMatch(name);
